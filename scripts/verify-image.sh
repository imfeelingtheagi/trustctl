#!/usr/bin/env bash
#
# Verify a published certctl image before you run it: confirm its keyless cosign
...
#
# Usage: scripts/verify-image.sh ghcr.io/imfeelingtheagi/certctl:<tag>
set -euo pipefail

image="${1:-}"
if [ -z "$image" ]; then
  echo "usage: $0 <image-ref>   e.g. ghcr.io/imfeelingtheagi/certctl:v0.1.0" >&2
  exit 2
fi
...
identity_re='^https://github.com/.*/certctl/.github/workflows/release.yml@.*'
...
echo ">> ${image} is signed by the certctl release workflow and ships an SBOM"
