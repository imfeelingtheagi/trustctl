# trustctllint — the architecture linter

`trustctllint` is a custom [`go/analysis`](https://pkg.go.dev/golang.org/x/tools/go/analysis)
multichecker that enforces trustctl's architectural non-negotiables in CI. A pull
...
```bash
go run ./tools/trustctllint ./...          # whole module (what `make lint` runs)
go run ./tools/trustctllint -tenantfilter=false ./...   # disable one rule for a run
go vet -vettool=$BIN ./...                # also usable as a vet tool
```

Each analyzer is independently testable with `go test ./tools/trustctllint/...`,
