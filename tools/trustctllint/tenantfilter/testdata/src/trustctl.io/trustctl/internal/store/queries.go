...
const (
	...
	setTenant = "SELECT set_config('trustctl.tenant_id', $1, true)"
...
func systemException() string {
	//trustctl:system-query — enumerates all tenants for a system maintenance pass; no tenant predicate by design.
...
func incidentalMarkerMention() string {
	// we deliberately avoid the trustctl:system-query hatch here
