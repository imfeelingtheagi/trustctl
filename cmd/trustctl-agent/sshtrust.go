...
import (
	...
	"trustctl.io/trustctl/internal/agent/sshtrust"
)
...
func runSSHTrustAddCA(ctx context.Context, o sshTrustOptions) (handled bool, err error) {
	...
	if changed {
		fmt.Printf("trustctl-agent: SSH CA trust added to %s (sshd validated, reloaded, health-checked; auto-rollback armed)\n", o.trustedKeys)
	} else {
		fmt.Printf("trustctl-agent: SSH CA already trusted in %s (no change)\n", o.trustedKeys)
	}
	return true, nil
}
...
func (osFS) WriteFileAtomic(p string, data []byte, mode os.FileMode) error {
	dir := filepath.Dir(p)
	tmp, err := os.CreateTemp(dir, ".trustctl-sshtrust-*")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
