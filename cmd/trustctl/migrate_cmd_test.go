...
// TestRun_MigrateStatusRequiresExternalDatastore: trustctl --migrate-status
...
func TestRun_MigrateStatusRequiresExternalDatastore(t *testing.T) {
	...
	if err == nil {
		t.Fatal("trustctl --migrate-status with bundled Postgres should fail fast (external datastore required)")
	}
}
...
func TestRun_MigrateRequiresExternalDatastore(t *testing.T) {
	...
	if err == nil {
		t.Fatal("trustctl --migrate with bundled Postgres should fail fast (external datastore required)")
	}
}
