...
// TestRun_BackupRequiresExternalDatastores: certctl --backup against the default
...
func TestRun_BackupRequiresExternalDatastores(t *testing.T) {
	...
	if err == nil {
		t.Fatal("certctl --backup with embedded NATS should fail fast (external datastores required)")
	}
}
