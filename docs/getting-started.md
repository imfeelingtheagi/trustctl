...
waiting on trustctl. The control plane is serving about two minutes after
...
trustctl ships a one-command evaluation stack — the control plane plus PostgreSQL
and NATS JetStream:
...
!!! tip "Transport encryption"
    ...
    `server.tls.mode=file` with your own certificate (`TRUSTCTL_SERVER_TLS_CERT_FILE`
    / `TRUSTCTL_SERVER_TLS_KEY_FILE`). Only set `server.tls.mode=disabled` for local
...
!!! note "Two ways to evaluate: the single binary, or Compose"
    The `trustctl` binary runs a **complete single-node evaluation stack out of the
    box** — bundled single-node PostgreSQL (`TRUSTCTL_POSTGRES_MODE=bundled`, the
    default) and embedded NATS (`TRUSTCTL_NATS_MODE=embedded`, the default), no
    external services required. For **production**, point it at managed datastores:
    `TRUSTCTL_POSTGRES_MODE=external` / `TRUSTCTL_POSTGRES_DSN` and
    `TRUSTCTL_NATS_MODE=external` / `TRUSTCTL_NATS_URL` — exactly what the Compose
...
In **Connect a CA**, give your issuer a name and continue. trustctl brokers
issuance to the CA you connect; this first issuer is all you need to proceed.

### Install an agent

In **Install an agent**, trustctl mints a one-time bootstrap token and shows the
exact command to run on a host inside your network:

```bash
trustctl-agent enroll --server https://localhost:8443 --token <BOOTSTRAP_TOKEN>
```
...
for how to get the `trustctl-agent` binary on Linux, macOS, and Windows.

### Issue your first cert

In **Issue your first cert**, name the service the certificate belongs to and
click **Issue**. trustctl creates the owner and identity and issues the
certificate through the CA you connected. You will see a confirmation and a link
to the certificate inventory.

That is your first certificate — discovered, owned, and tracked. trustctl will now
rotate and renew it automatically.

!!! note "Measured issuance time"
    Issuance is fast. In trustctl's end-to-end integration test — the assembled
...
issuer=$(echo '{"kind":"x509_ca","name":"Primary CA"}' | trustctl-cli issuers create -f - | jq -r .id)
ident=$(echo "{\"kind\":\"x509_certificate\",\"name\":\"payments.svc\",\"owner_id\":\"$owner\",\"issuer_id\":\"$issuer\"}" \
          | trustctl-cli identities create -f - | jq -r .id)

# Transition it to "issued": the running outbox dispatcher mints the certificate.
echo '{"to":"issued"}' | trustctl-cli identities transition "$ident" -f -
sleep 2

# The newly minted certificate is now in inventory.
trustctl-cli certificates list
```
