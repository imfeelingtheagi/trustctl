...
func TestMigrateAutoEnvOverride(t *testing.T) {
	...
	if cfg.Migrate.Auto {
		t.Error("CERTCTL_MIGRATE_AUTO=false should disable automatic migration")
	}
}
...
func TestMigrateAutoMalformedEnvIgnored(t *testing.T) {
	...
	if !cfg.Migrate.Auto {
		t.Error("a malformed CERTCTL_MIGRATE_AUTO should be ignored, leaving auto-migrate on")
	}
}
