...
-- The hot claim path uses a pending-row partial index; delivered rows never match
-- that index. RLS and grants are inherited from 0003 (outbox); the sweep is a
-- cross-tenant system operation on the pool (RLS-bypassing), like the dispatcher
-- and the idempotency-key GC.
--
